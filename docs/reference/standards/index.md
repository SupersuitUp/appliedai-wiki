...
Four specs sit in this section. All are agent-readable file formats. All are versioned, dated, and meant to be copied into your own repos.
...
- **[brand.txt](/reference/standards/brand-txt)**. A priming format. One statically-served file that makes a brand agent-ready in a single link: identity, master prompt, tokens, voice, and banned terms inlined, plus absolute URLs to every brand asset. Like `llms.txt` for a website, but for generating in a brand's voice and look. Generated at build time from an [AI-Native Brand OS](/concepts/ai-native-brand-os).

All four formats target an agent reader. All are intentionally minimal at v0.x. All evolve based on real usage patterns rather than upfront design.
