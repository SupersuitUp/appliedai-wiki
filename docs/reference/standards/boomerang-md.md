...
- **[Bug Report Boomerang](pathname:///skills/bug-report/BOOMERANG.md).** A boomerang that interviews whoever hit a bug in something you built and returns a reproducible bug report you paste into a coding agent. Notable for inverting the generous frame: the device here is a meticulous QA lead forbidden from diagnosing, because a consumer AI that starts theorizing mid-interview biases every answer after it. Its Part 3 is a guided browser-console capture, and it is scoped one run per bug rather than per subject.

All four are conforming instances of this spec: the same two-part contract, the same two levers, different areas and different return schemas.
