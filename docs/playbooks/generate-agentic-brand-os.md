...
image: "/img/comics/generate-a-brand-os.webp"
---
...
6c. **Promote load-bearing entities to Canon OS dossiers.** The flat `characters` block routes agents to the right GABR, but everything else production depends on (hard visual rules, wardrobe eras, multi-angle reference sets, verification checklists) otherwise lives scattered across prose fields that drift apart, and hand-maintained prompt descriptor strings drift from the locked references, which is exactly how continuity failures happen. For any recurring visual entity that MUST stay consistent across many generations (a mascot, a spokesperson, a signature location, a hero prop), create a folder dossier at `static/brand/canon/<slug>/` holding a `manifest.json` plus that entity's reference images co-located. Manifest core: `slug`, `type` (`person` | `place` | `prop`), `name`, `role`, an `authority` block (`locked_by`, `locked_on`, optional `origin` naming where the canon originates; only the brand owner relocks a dossier; a render that disagrees with the dossier is a defective render, never the reverse), `identity` (`form` prose + `hard_rules`), `references` (role-named files), `angles` (which references MUST be passed per view: front/back/side; multi-angle turnarounds keep merch and back-views consistent), `verification` (the crop-zoom read-back checklist), and `never` (the banned-drift list). Type-specific blocks: `person` adds `wardrobe` eras and `voice`; `place` adds `geometry`/`lighting`/`population` plus `blocking` (the default seating/position chart for the recurring cast) and `dressing` (always-present props with fixed positions, e.g. a table that must be on set every group scene); `prop` adds `construction`/`scale`. For a recurring group set, geometry alone is not enough: the model still re-improvises *who sits where* and *which props are out* until it reads amateur; `blocking` and `dressing` compile into the place descriptor, and the rule is to **pass the set's establishing reference, state its blocking + dressing in the prompt (the reference alone will not hold them), and verify both on read-back**. Lock the set + default blocking, not the camera: framing stays free; the dressing and default seats hold unless a scene deliberately re-blocks. Activate with one key in `brand.json`: `"canon": { "root": "static/brand/canon/" }`. The build script discovers every folder containing a `manifest.json`, validates it loudly (a missing field or a reference filename that does not exist FAILS the build), and compiles each entity into a canonical descriptor in `brand.txt`: agents paste descriptors verbatim and never hand-write them. The flat `characters` block stays valid forever; Canon OS is additive, adopted per-entity when an entity earns it. The canonical implementation (schema, validator, compiler, starter example) ships in the brand-os template repo (`scripts/canon.mjs`, `scripts/canon-manifest.schema.json`, `static/brand/canon/example-mascot/`).
...
7b. **Wire `brand.txt`, generated at build time.** A small dependency-free generator (Node, on `prebuild`/`prestart`) reads `brand.json`, inlines identity + preamble + tokens + voice + banned terms, and emits an absolute URL for every asset, writing `static/brand.txt`. It declares its format and links the [brand.txt standard](/reference/standards/brand-txt).
   ...
   **The generator must also emit a `## Canon (compiled descriptors — paste verbatim, never paraphrase)` section** when `brand.json` carries the `canon` key. For each dossier it compiles: one DESCRIPTOR paragraph per wardrobe era (identity form + hard rules + era + the NEVER list), a PASS line resolving the `angles` map to served reference URLs, a VERIFY-on-read-back line from the `verification` checklist, and the authority line (who locked it, when, and its origin if inherited). Validation failures fail the build with per-entity messages: a broken dossier can never ship silently. This is what kills descriptor drift: the paste-verbatim string is compiled from the same locked data the references live beside, never hand-maintained.
...
- **Hand-maintained prompt descriptors.** Copy-pasted character description strings inside batch scripts and prompt files drift from the locked reference sheet one edit at a time, until a render "corrects" the reference instead of the render. Descriptors must be compiled from the canon dossier by the build script and pasted verbatim from `brand.txt`; an agent hand-writing (or hand-"fixing") a descriptor is the failure mode, not a convenience.
