...
|---|---|---|
| **save-my-progress** | A reviewed sweep that rescues a session's value (work log, [skills](/concepts/skill-files), wiki/docs, README, memory, commit, resume note, report-back), proposing a checklist and confirming before it writes anything. The save mechanic for the [playable harness experience](/concepts/playable-harness-experience). See [The Session Save](/concepts/the-session-save). | [`SKILL.md`](https://www.appliedai.wiki/skills/save-my-progress/SKILL.md) · [`GENERATE.md`](https://www.appliedai.wiki/skills/save-my-progress/GENERATE.md) (factory: installs a copy personalized to your workspace) |
...
| **open-clauded-tab** | Opens a new terminal tab in the VS Code window you are already in and starts a fresh interactive agent session there, optionally in a given directory with the first prompt already typed. Useful when work should continue in a sibling session you can talk to rather than a subagent you cannot. macOS and VS Code only. See [Remote Control Is Incomplete Without Remote Boot](/perspectives/remote-control-is-incomplete-without-remote-boot). | [`SKILL.md`](https://www.appliedai.wiki/skills/open-clauded-tab/SKILL.md) · [`open-tab.sh`](https://www.appliedai.wiki/skills/open-clauded-tab/open-tab.sh) |
| **start-agentic-project** | Stands ONE project up as a repo with a state machine, verbs and a supervising subagent, so closing the laptop costs nothing and reopening it costs one command. Interviews you first, then does the cloning and filling itself. See [Agentic Project Management](/concepts/agentic-project-management). | [`GENERATE.md`](https://www.appliedai.wiki/generators/start-agentic-project/GENERATE.md) (the recipe: hand an agent this URL) · [template repo](https://github.com/SupersuitUp/agentic-project-template) (the payload) · [`BOOMERANG.md`](https://github.com/SupersuitUp/agentic-project-template/blob/master/BOOMERANG.md) (same interview, for someone else's project) |
| **create-feedback-receipt** | Turns a round of feedback into a published page that answers every item with a before and after, including the ones you declined, then sends it back with an ask for the next pass. The cheapest way to make a good reviewer review you again. | [`SKILL.md`](https://www.appliedai.wiki/skills/create-feedback-receipt/SKILL.md) |
| **lift-all-boats** | Propagates a fix across every instance of a family AND into the template that mints new ones, so the defect cannot come back. Ships each fix as a detector plus a remedy, because a remedy-only sweep silently no-ops on instances that drifted and reports success everywhere. See [Fixing the Generator Does Not Fix the Fleet](/perspectives/fixing-the-generator-does-not-fix-the-fleet). | [`SKILL.md`](https://www.appliedai.wiki/skills/lift-all-boats/SKILL.md) |
...
- [The Session Save](/concepts/the-session-save): the concept behind the first hosted skill.
