...
image: "/img/comics/boomerang-prompt.webp"
---
...
**The bug report.** The same shape, retargeted from a person to a moment. Someone tests what you built and tells you "it doesn't show up." Between that sentence and a defect an agent can fix sit a dozen questions the reporter does not know to answer and you do not want to ask one text message at a time. The Boomerang Prompt interviews them across thirteen areas (exact URL, every step in order, expected against actual, device and browser versions, signed-in state, what a refresh changed, who else has seen it) and returns a reproducible bug report you paste into your harness, led by a triage TL;DR and closed by a required unknowns list. It inverts the generous frame: the device is a QA lead explicitly forbidden from diagnosing, because an AI that starts theorizing mid-interview biases every answer after it. The full paste-in prompt ships as a conforming [BOOMERANG.md instance](pathname:///skills/bug-report/BOOMERANG.md).
