...
Holding the pace has a companion move at each model release. Guardrails are scaffolding around the current model's bugs: [the more capable the agent, the more guardrails it needs](/perspectives/capable-agents-need-more-guardrails), and each guardrail is dated the day it is written. An image pipeline might carry a checker whose only job is catching a rendering mistake the current image model makes constantly; the next model release makes that checker deletable. So budget a **deletion pass** per model release: treat every new model as a new car and re-learn where its edges are before trusting last year's driving habits. The public receipt is Boris Cherny reporting that the Claude Code team deleted over 80 percent of the system prompt when Opus 5 landed, and the model performed better ([YC interview, 2026-07-28](https://www.ycrootaccess.com/p/boris-cherny-building-claude-code)). A team at the Goldilocks pace deletes on schedule; a team running too hot keeps stacking scaffolding it no longer needs.
...
- [The More Capable the Agent, the More Guardrails It Needs](/perspectives/capable-agents-need-more-guardrails) holds the tension the deletion pass resolves.
