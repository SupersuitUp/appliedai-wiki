...
The principle is a clean division of labor: the recurring identity rides in stable artifacts — tokens, reference images ([Golden Atomic Brand References](/concepts/golden-atomic-brand-references)), components — and the variable per-asset content rides in the prompt or the props. The Agentic Brand OS applies this discipline to the whole brand: copy and code included, not just visuals.
