...
**Read the old checkpoint for SCOPE, never for FACTS.** It tells you where to start diffing. Every claim inside it about the current state of the world ("repo X has no remote", "Y is unpushed", "Z is still unbuilt") was true when written and may not be now, and a save that copies those forward writes a confident lie into the record the next session will trust. Re-derive each one from the system itself before repeating it: `git remote -v`, `git rev-list --left-right --count origin/<branch>...HEAD`, `gh repo view`, `git status --porcelain`. A checkpoint's authority is its SHA, nothing else.
...
- **Route each lesson BEFORE writing it.** Ask *when does this need to fire?* and write it to the NARROWEST home that fires then: only during a specific task → that task's skill; only inside one repo → that repo's `CLAUDE.md`; about a person → their PRM file; genuinely unpredictable trigger → `~/.agents/AGENTS.md`, as **one line** that states the rule and points at the skill holding the reasoning. **`AGENTS.md` is the last resort, not the default.** It loads into every session forever, so a rule there is paid for on every future turn whether or not it is relevant; it was cut from 52k to 13k chars on 2026-07-21 precisely because sessions kept promoting task-specific lessons into it. If you can name the moment a rule fires, it does not belong there. **Two or more AGENTS.md additions in one session is a smell** that you are routing by importance instead of by trigger.
- **Sweep for hand-rolled work → `abu:pave-the-path`.** Before writing any lesson down, look at what
  the session actually IMPROVISED: every `*.sh`/`*.py` in the scratchpad, every retry loop, every check you wrote
  by hand, everything you did N times manually. Those are the highest-leverage items in a save, and they are the
  ones a survey of the git diff alone will miss entirely, because throwaway scripts live outside the repo. The
  skill's bar is a naming sentence ("I hand-rolled X and the next invocation that needs X is Y") and it BUILDS
  what clears it rather than filing a suggestion. A note describing a hand-roll is not a fix: prose changes
  nothing about what the next run does.
